...
TRAIN_RECS ?= $(shell bash -c 'read -p "Number of train images per tfrecord (default: 107)? " num_train_records; echo $$num_train_records')
VAL_RECS ?= $(shell bash -c 'read -p "Number of validation images per tfrecord (default: 5)? " num_val_records; echo $$num_val_records')
IMG_SIZES ?= $(shell bash -c 'read -p "Image height and width (default: 224, 224)? " image_size; echo $$image_size')
tfrecords: scripts/create_tfrecords.sh
	echo "Creating tfrecords..."
	@bash scripts/create_tfrecords.sh -d $(IMG_DIR) -p $(TFREC_DIR) -t $(TRAIN_RECS) -v $(VAL_RECS) -s $(IMG_SIZES)
