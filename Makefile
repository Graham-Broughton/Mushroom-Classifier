...
.PHONY: all build dotenv init all_datasets datasets_of_interest fgvcx_2018 fgvcx_2019 fgvcx_2021 \
preprocess_data tfrecords get_deploy_model local_deploy cloud_run_build cloud_run_deploy cloud_run_delete help
...
cloud_run_deploy: cloud_run_build
	@: $(eval VERSION := $(shell bash -c 'read -p "What version should it be tagged? " version; echo $$version'))
	@cd mush_app && gcloud run deploy mushroom-classifier --image="${GCR_TAG}:${VERSION}" --max-instances=1 --min-instances=0 --port=8080 \
...
help:
	...
	@echo "datasets_of_interest: 			- Download only the years of interest, use -j2 for parallelization"
	...
	@echo "local_deploy:            		- Deploy the model to a local server using ngrok"
