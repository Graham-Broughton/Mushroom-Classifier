...
def join_dataframes(
    images, annotations, categories, dset=None, locations=None, is2019=False
):
    ...
    cats = cats[cats["supercategory"] == "Fungi"].rename(columns={"id": "category_id"})
    if is2019:
        df = (
            pd.merge(
                annotations,
            ...
            .set_index("image_id")
            .merge(images, left_index=True, right_index=True)
        )
    elif locations is None:  # some datasets do not have location information
        df = pd.merge(cats, annotations, right_on="category_id", left_index=True).merge(
            images, left_on="image_id", right_index=True
        )
    else:
        df = pd.merge(annotations, cats, on="category_id").set_index("image_id")
...
def parse_2018_data(data_root):
    ...
    (timages2018, tanno2018), (vimages2018, vanno2018) = [
        parse_json(data_root / f"{s}2018.json")[1:] for s in ["train", "val"]
    ]

    # Parse train and validation locations
    tloc, vloc = [
        pd.read_json(
            data_root / "inat2018_locations" / f"{s}2018_locations.json"
        ).set_index("id")
        for s in ["train", "val"]
    ]
...
def parse_2019_data(data_root):
    ...
    Returns:
        DataFrame: A dataframe containing the parsed data.
    """
    logger.info(f"Parsing 2019 data from {data_root}")

    # Parse categories
    with open(data_root / "categories.json", "r") as f:
        cats = pd.DataFrame(json.load(f))
    cats.rename({"kingdom": "supercategory"}, axis=1, inplace=True)

    # Parse train and validation data
    (timages2019, tanno2019), (vimages2019, vanno2019) = [
        parse_json(data_root / f"{s}2019.json")[1:] for s in ["train", "val"]
    ]

    # Join dataframes and save which dset they are from
    val = join_dataframes(vimages2019, vanno2019, cats, dset="val", is2019=True)
    train = join_dataframes(timages2019, tanno2019, cats, dset="train", is2019=True)
    df = pd.concat([train, val]).reset_index(drop=True)
    df["dataset"] = "2019"
    ...
    # Drop unneeded columns and rename others
    df = df.drop(["category_id"], axis=1).rename(
        columns={
            "lon": "longitude",
            "lat": "latitude",
            "loc_uncert": "location_uncertainty",
    ...
    logger.debug(f"2019 dataframe shape {df.shape}")
    return df
...
def join_datasets(root) -> tuple:
    ...
    df2.rename({"supercategory": "kingdom"}, axis=1, inplace=True)
    df3.drop(
        [
            "image_id",
            "location_uncertainty",
    ...
    df.drop(["license", "rights_holder"], axis=1, inplace=True)
    ...
    df["class_id"] = df["name"].cat.codes
    # df["date"] = pd.to_datetime(df["date"], format="mixed", utc=True)
    # df["gcs_path"] = df.apply(
    #     lambda x: f"gs://{CFG.GCS_REPO}/data/raw/{x['dataset']}/{x['phylum']}_{x['class']}_{x['order']}_{x['family']}_{x['name']}/{x['file_name']}",
    #     axis=1,
    # )
...
if __name__ == "__main__":
    ...
    df = join_datasets(raw_data_root)
