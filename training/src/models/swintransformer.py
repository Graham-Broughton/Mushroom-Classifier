...
class SwinTransformerModel(tf.keras.Model):
    ...
    def predict_step(self, data):
        x, y, z = data
        return self(x, training=False), y, z
...
def SwinTransformer(model_name='swin_tiny_224', num_classes=1000, include_top=True, pretrained=True, use_tpu=False, cfgs=CFGS):
    ...
    if pretrained_ckpt:
        ...
        if use_tpu:
            load_locally = tf.train.CheckpointOptions(experimental_io_device="/job:localhost")# tf.saved_model.LoadOptions(experimental_io_device='/job:localhost')
