...
def load_dataset(filenames, CFG, labeled=True, ordered=False):
    ...
    )  # if labeled else read_unlabeled_tfrecord
...
def get_training_dataset(training_filenames, CFG):
    dataset = load_dataset(training_filenames, CFG, labeled=True)
...
def get_validation_dataset(validation_filenames, CFG, ordered=False):
...
def count_data_items(filenames):
    ...
    n = [
        int(compile(r"-([0-9]*)\.").search(filename).group(1)) for filename in filenames
    ]
    return np.sum(n)
